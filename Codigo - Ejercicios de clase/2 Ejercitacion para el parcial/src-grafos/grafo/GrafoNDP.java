...
public class GrafoNDP {
	...
	private void colorear() {
		...
		nodosColoreados[this.nodos.get(0).getNumero()] = color; // coloreo el
																	// primer
																	// nodo de
																	// la lista

		for (int i = 1; i < this.cantNodos; i++) {
			nodo = this.nodos.get(i).getNumero();
			this.nodosColoreados[nodo] = color;
			j = 0;
			while (j < this.cantNodos) {
				if (nodo != j) {
					if (this.grafo.getPos(nodo, j) && this.nodosColoreados[nodo] == this.nodosColoreados[j]) {
						color++;
						if (color > colorMax)
							colorMax = color;
						this.nodosColoreados[nodo] = color;
	...
	public void mostrarNodos() {
		for (int i = 0; i < this.cantNodos; i++)
			System.out.println("n: " + this.nodos.get(i).getNumero() + " g: " + this.nodos.get(i).getGrado());
	}
