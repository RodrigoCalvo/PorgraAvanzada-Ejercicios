...
public class Kruskal {
	...
	public void ejecutar() throws IOException {
		...
		for (int i = 0; i < this.cantNodos; i++) {
			for (int j = i + 1; j < this.cantNodos; j++) {
				...
				if (this.grafo.getGrafo().getPos(i, j)) {
		...
		for (AristaPonderada arista : this.aristas) {
			...
			if (find(nodo1) != find(nodo2)) {
				...
				union(nodo1, nodo2);
				// agrego la arista al arbol abarcador mÃ­nimo
				this.arbol.add(arista);
				// acumulo el peso de la arista al costo mÃ­nimo
	...
	public void escribirSolucionEnConsola() {
		...
		System.out.println("Costo del Ã?rbol Abarcador MÃ­nimo: " + this.minimoCosto);
