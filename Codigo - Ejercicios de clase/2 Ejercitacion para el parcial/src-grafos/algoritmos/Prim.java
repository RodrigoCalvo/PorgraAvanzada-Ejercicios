...
public class Prim {
	...
	public void ejecutar(String path) throws IOException {
		...
		while (this.arbol.size() < this.cantNodos - 1) {
			...
			for (int nodoActual : this.nodosConectados) {
				...
				for (int nodo : this.nodosNoConectados) {
					...
					if (this.grafo.getGrafo().getPos(nodoActual, nodo)) {
		...
		this.escribirSolucionEnArchivo(path + "PRIM" + "_" + this.cantNodos + "_"
	...
	public void escribirSolucionEnConsola() {
		...
		System.out.println("Costo del Ã?rbol Abarcador Mínimo: " + this.minimoCosto);
