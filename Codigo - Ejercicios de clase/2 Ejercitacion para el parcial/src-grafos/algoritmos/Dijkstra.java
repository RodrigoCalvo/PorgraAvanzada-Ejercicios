...
public class Dijkstra {
	...
	public void ejecutar(String path) {
		...
		while (!cola.isEmpty()) {
			...
			for (int i = 0; i < this.cantNodos; i++) {
				...
				if (nodo != i && !this.nodoTerminado[i]) {
					...
					if (this.grafo.getGrafo().getPos(nodo, i)) {
		...
		this.escribirSolucionEnArchivo(path + "DIJKSTRA" + "_" + this.cantNodos + "_"
	...
	private void escribirSolucionEnArchivo(String path) {
		...
		try {
			...
			for (CostoAlNodo nodoFinal : costos) {
				pw.println(this.getNodoInicial() + " " + nodoFinal.getNodo() + " " + nodoFinal.getCostoMinimo() + " " 
						+ nodoFinal.getCaminoMasCorto().size());
				for (Integer nodoDelCamino : nodoFinal.getCaminoMasCorto()) {
					pw.print(" " + nodoDelCamino);
				}
			}
